...
- Eden's native random starting passive is returned to its original item pools
  ...
  again. The internal reset finishes while the starting room is black, then
  matches the normal fade-in timing without showing a rewind
  transition. In place of the passive, three random collectible pedestals
  appear, Eden starts centered below them, and only one can be taken.
